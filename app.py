...
def load_models():
    try:
        model_throughput = joblib.load("models/throughput_model.pkl")
        model_risk = joblib.load("models/risk_classifier.pkl")
        return model_throughput, model_risk
    except:
        st.warning("Models not found. Using simulated predictions.")
        return None, None
...
# Prediction with safe feature handling
if model_throughput is not None:
    # Create input with exact feature names the model expects
...
with tab3:
    ...
    if st.button("📥 Download Full Executive Report (Excel)"):
        ...
        st.download_button("Click to Download", output.getvalue(), "Warehouse_Executive_Dashboard.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
